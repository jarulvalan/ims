            ...
            for items in products:
                ...
                if name==items.name and cetagory==items.cetagory and supplier==items.supplier:
                    if not request.POST._mutable:
                        ...
                        #print(items.id)
                        request.POST['name']=items.name
                        request.POST['cetagory']=items.cetagory
                        request.POST['supplier']=items.supplier
                        request.POST['id']=items.id
                        product = get_object_or_404(Product, pk=items.id)
